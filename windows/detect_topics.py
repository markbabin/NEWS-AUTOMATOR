...
def detect_topics(
    ...
    # Deduplicate, merge overlapping segments, round timestamps, and add padding
    for topic_name in topic_names:
        merged = merge_segments(results[topic_name])
        for seg in merged:
            seg["start"] = round_to_5s(seg["start"])
            seg["end"] = round_to_5s(seg["end"])
            # Pad: subtract 5s from start, add 5s to end (better to capture extra than miss content)
            start_secs = max(0, to_seconds(seg["start"]) - 5)
            end_secs = to_seconds(seg["end"]) + 5
            seg["start"] = to_hms(start_secs)
            seg["end"] = to_hms(end_secs)
