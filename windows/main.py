...
from paths import get_bundle_dir, get_user_data_dir, ensure_directories, ensure_config
...
def process_video(
    ...
    metadata_callback: optional callable(video_path) -> (date, channel, show) or None.
                       Used by the GUI to show a dialog instead of stdin input().
    """
    ...
    if meta is None:
        if metadata_callback:
            meta = metadata_callback(video_path)
        else:
            meta = prompt_for_metadata(video_path)
        if meta is None:
            print(f"  Skipped {video_path.name} — no metadata provided.")
            return 0
...
def main():
    ...
    if args.config != "config.yaml":
        ...
        config_path = ensure_config(base_dir)
