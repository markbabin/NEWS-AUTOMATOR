...
MONO_FONT = "Consolas" if platform.system() == "Windows" else MONO_FONT
...
class MetadataDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, filename: str):
        ...
        ctk.CTkLabel(self, text=filename, font=(MONO_FONT, 12), anchor="w").pack(
...
class App(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        self.grid_rowconfigure(5, weight=1)  # log area gets the stretch
        ...
        mode_frame.grid(row=2, column=0, padx=16, pady=4, sticky="ew")
        mode_frame.grid_columnconfigure(1, weight=1)

        ctk.CTkLabel(mode_frame, text="Mode:").grid(row=0, column=0, padx=(0, 8))
        self._config_map = self._find_configs()
        self.mode_var = ctk.StringVar(value=list(self._config_map.keys())[0])
        self.mode_menu = ctk.CTkOptionMenu(mode_frame, variable=self.mode_var,
                                            values=list(self._config_map.keys()), width=250)
        self.mode_menu.grid(row=0, column=1, sticky="w")

        # --- Row 3: Options + buttons ---
        opts_frame = ctk.CTkFrame(self, fg_color="transparent")
        opts_frame.grid(row=3, column=0, padx=16, pady=(4, 4), sticky="ew")
        ...
        self.progress_frame.grid(row=4, column=0, padx=16, pady=(0, 4), sticky="ew")
        ...
        # --- Row 5: Log area ---
        self.log_box = ctk.CTkTextbox(self, state="disabled", font=(MONO_FONT, 12))
        self.log_box.grid(row=5, column=0, padx=16, pady=(0, 8), sticky="nsew")

        # --- Row 6: Bottom buttons ---
        bottom = ctk.CTkFrame(self, fg_color="transparent")
        bottom.grid(row=6, column=0, padx=16, pady=(0, 16), sticky="ew")
        ...
        if not shutil.which(ffmpeg) and ffmpeg == "ffmpeg":
            ...
            self._log("  Windows: download from ffmpeg.org\n")

    # ── Config discovery ────────────────────────────────────────

    def _find_configs(self) -> dict[str, Path]:
        """Find all config*.yaml files and return {display_name: path} map."""
        configs = {}
        # Check both bundle dir (for packaged app) and user data dir
        search_dirs = [get_bundle_dir()]
        if get_user_data_dir() != get_bundle_dir():
            search_dirs.append(get_user_data_dir())
        for d in search_dirs:
            for f in sorted(d.glob("config*.yaml")):
                if f.name == "config.yaml":
                    configs["News Monitor"] = f
                elif f.name == "config_skijumping.yaml":
                    configs["Ski Jumping"] = f
                else:
                    # Generic: strip config_ prefix and .yaml suffix
                    label = f.stem.replace("config_", "").replace("_", " ").title()
                    configs[label] = f
        if not configs:
            configs["News Monitor"] = get_bundle_dir() / "config.yaml"
        return configs
    ...
    def _run_pipeline(self, api_key: str, videos: list[Path]):
        ...
        try:
            ...
            config_path = self._config_map.get(selected_mode)
            if config_path is None or not config_path.exists():
                config_path = ensure_config(DATA_DIR)
            config = load_config(config_path)
            print(f"Using config: {config_path.name} ({selected_mode})")
